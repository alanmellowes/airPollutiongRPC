...
import calcAirPoll.AirCalcServiceGrpc.AirCalcServiceImplBase;

import io.grpc.Server;
import io.grpc.ServerBuilder;
import io.grpc.stub.StreamObserver;

public class AirCalcServer extends AirCalcServiceImplBase {


	public static void main(String[] args) {
		AirCalcServer mathserver = new AirCalcServer();
		...
		try {
			...
			System.out.println("Air Pollution Server started, listening on " + port);
	...
	private Properties getProperties() {
		...
		Properties prop = null;		
		 ...
		 try (InputStream input = new FileInputStream("src/main/resources/calcAirService.properties")) {
	...
	private  void registerService(Properties prop) {
		 ...
		 try {
	           ...
	           // int service_port = 1234;
	            int service_port = Integer.valueOf( prop.getProperty("service_port") );// #.50090;
	...
	public void calculate(CalculateRequest request, 
		...
		System.out.println("receiving compute method " + request.getNumber1() + " , " + request.getNumber2() + " " + request.getOperation() );

		float value = Float.NaN;
		String msg= "ok " + request.getOperation().name() + " result ";

		if(	request.getOperation()==Operation.TOTAL)
			value = request.getNumber1() + request.getNumber2();
		else if(	request.getOperation()==Operation.DIFFERENCE)
			value = request.getNumber1() - request.getNumber2();
	...
	public void generateRandomNumbers(RandomRequest request,
		...
		System.out.printf("receiving random air pollution levels across the globe method num: %d from: %d to: %d \n",
	...
	public StreamObserver<NumberMessage> averageValues(
		...
		return new StreamObserver<NumberMessage>() {
			...
			public void onNext(NumberMessage request) {
				...
				System.out.println("receiving average Air Pollution levels: "+ request.getNumber()  );
			...
			public void onCompleted() {
				System.out.printf("receiving Air Pollution levels method complete \n" );
